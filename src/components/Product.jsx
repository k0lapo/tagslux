...
const Product = ({ product }) => {
  ...
  return (
    <div className="max-w-sm bg-white rounded overflow-hidden shadow-lg m-2 transform hover:scale-105 transition-transform duration-300 grid gap-8">
      <Link to={`/product/${product.id}`}>
        <div className="relative w-full h-48">
          <img
            className="w-full h-48 object-cover transition-opacity duration-300 front-image"
            src={product.image}
            alt={product.name}
          />
          {product.backImage && (
